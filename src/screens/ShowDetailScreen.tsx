...
export const ShowDetailScreen = () => {
  ...
  console.log("🚀 ~ ShowDetailScreen ~ show:", show);
  const {
  ...
  const loadAd = async () => {
    try {
      ...
      const cityFilteredAds = allAds.data.filter(
        (ad) => ad.city === selectedCity || ad.city === "Both"
      );
  ...
  const handleAddToCalendar = () => {
    ...
    try {
      ...
      const eventDetails = encodeURIComponent(
        `${show.genre} show. ${show.description || ""}`
      );
      ...
      Linking.openURL(googleCalendarUrl).catch(() => {
        MagicallyAlert.alert(
          ...
          "Please add the event manually to your calendar."
        );
      });
    } catch (error) {
              ...
              {show.price && (
                <View
                  style={{
                    flexDirection: "row",
                    alignItems: "flex-start",
                    marginBottom: show.sourceUrl ? 16 : 0,
                  }}
                >
                  <View
                    style={{
